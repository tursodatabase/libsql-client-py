...
async def test_named_args(client, sign):
    rs = await client.execute(f"SELECT {sign}b, {sign}a", {"a": "one", f"{sign}b": "two"})
    assert tuple(rs.rows[0]) == ("two", "one")

@pytest.mark.asyncio
@pytest.mark.parametrize("sign", [":", "@", "$"])
async def test_named_args_used_multiple_times(client, sign):
    rs = await client.execute(f"SELECT {sign}b, {sign}a, {sign}b || {sign}a", {"a": "one", f"{sign}b": "two"})
    assert tuple(rs.rows[0]) == ("two", "one", "twoone")

@pytest.mark.asyncio
@pytest.mark.parametrize("sign", [":", "@", "$"])
async def test_named_args_and_pos_args(client, sign):
    rs = await client.execute(f"SELECT {sign}b, {sign}a, ?1", {"a": "one", f"{sign}b": "two"})
